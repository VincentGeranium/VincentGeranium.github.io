...
- ```swift showViewController:sender:```와 ```swift showDetailViewController:sender:```메서드는 뷰 컨트롤러를 표시하는데에 가장 적응성이 우수하고 유연한 방법을 제공한다
...
- ```swift presentViewController:animated:completion:```메서드는 뷰 컨트롤러를 항상 모달 방식으로 표시한다.
...
- [참고자료 : Using Segue](https://developer.apple.com/library/archive/featuredarticles/ViewControllerPGforiPhoneOS/UsingSegues.html#//apple_ref/doc/uid/TP40007457-CH15-SW1)
    ...
    - ```swift showViewController:sender:``` 또는 ```swift showDetailViewController:sender``` 메서드를 사용하여 뷰 컨트롤러를 나타낼 수 있다

        - 커스텀 뷰 컨트롤러의 경우, 이러한 메서드의 동작을 자신의 뷰 컨트롤러에 더욱 적합하도록 변경할 수 있다.
        
    - ```swift presentViewController:animated:completion:``` 메서드를 호출하여 뷰 컨트롤러를 모달로 나타낼 수 있다.
    
- - - 

### Showing View Controller (뷰 컨트롤러 보여주기)

```swift show(_:sender:)```와 ```swift showDetailViewController(_sender:)``` 메서드를 사용할 때, 새로운 뷰 컨트롤러를 화면에 띄우는 과정은 간단하다
...
- 4) 현재 뷰 컨트롤러의 ```swift showViewController:sender:```와 ```swift showDetailViewController:sender:``` 메서드를 호출

- UIKit은 ```swift showViewController:sender:```와 ```swift showDetailViewController:sender:``` 메서드에 대한 호출을 나타내는 뷰 컨트롤러(presenting view controller)에 전달한다.
